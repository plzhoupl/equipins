...
$(document).ready(function(){
	...
	// 第一校验组：同一校验组
	$("#travelAgencyName").formValidator({tipid:"travelAgencyNameText",validatorgroup:"1",onshow:"请输入被保险企业全称",oncorrect:"企业名称正确"}).inputValidator({min:2,max:100,onerror:"请确定输入的字符数在两个到一百个之间"});
	...
	$("#contactAddress").formValidator({tipid:"contactAddressText",validatorgroup:"1",onshow:"请输入联系人的通讯地址",oncorrect:"通讯地址输入正确"}).inputValidator({min:2,max:100,onerror:"请确定输入的字符数在两个到一百个之间"});
	$("#contactCellPhone").formValidator({tipid:"contactCellPhoneText",validatorgroup:"1",onshow:"请输入您的手机号码",onfocus:"手机号码必须为十一位",oncorrect:"谢谢您的合作"}).inputValidator({min:11,max:11,onerror:"手机号码必须是11位的,请确认"});
	...
	$("#province").formValidator({tipid:"unitText",validatorgroup:"1",onshow:"请点击选择所处地区，输入被保险人详细地址",oncorrect:"谢谢您的合作"}).inputValidator({min:1, onerror:"地区不允许为空"}) ;
	$("#city").formValidator({tipid:"unitText",validatorgroup:"1",onshow:"请点击选择所处地区，输入被保险人详细地址",oncorrect:"谢谢您的合作"}).inputValidator({min:1, onerror:"地区不允许为空"}) ;
	$("#contactEmail").formValidator({tipid:"contactEmailText",validatorgroup:"1",onshow:"请输入你的邮箱",onfocus:"请注意你输入的邮箱格式，例如:example@sina.com",oncorrect:"谢谢您的合作，你的邮箱格式正确"}).regexValidator({regexp:"email2",datatype:"enum",onerror:"邮箱格式不正确"}) ;
	$("#contactPostCode").formValidator({tipid:"contactPostCodeText",validatorgroup:"1",onshow:"请输入邮编",onfocus:"6位数字组成的哦",oncorrect:"谢谢您的合作，您的邮编正确"}).regexValidator({regexp:"zipcode2",datatype:"enum",onerror:"邮编格式不正确"});
	...
	// 第三校验组：事业单位
	$("#organizationCode").formValidator({tipid:"organizationCodeText",validatorgroup:"3",onshow:"请输入组织机构代码/个体经营者可输入个人身份证代码",oncorrect:"组织机构代码输入正确"}).inputValidator({min:9,max:18,onerror:"请确定输入的字符数是九到十八个字符"});

	// 第四校验组：企业单位
	$("#businessCode").formValidator({tipid:"businessCodeText",validatorgroup:"4",onshow:"请输入你的营业执照号码",onfocus:"请输入你的营业执照号码",oncorrect:"谢谢您的合作，你的营业执照号码格式正确"}).inputValidator({min:1,onerror:"营业执照号码格式不正确"}) ;
...
//            loadChinaUnitBeNull();
	...
	$("#tij").click(function () {
		if($("#checkbox").attr("checked")){
			...
			alert("请阅读并同意 《特种设备安全保险网注册条款》");
		}
	});

	$('input:[name="orgNature"]').click(function() {
		var orgNature = $("input:[name='orgNature']:checked").val();
		if(orgNature == 1){
			$("#orgNaturetr1").show();
			$("#orgNatureLtr1").show();
			...
			var organizationLicence=document.getElementById('organizationLicence');
			organizationLicence.innerHTML=organizationLicence.innerHTML.replace(/(value=\").+\"/i,"$1\"");
			$("#orgNatureLtr2").show();
			$("#orgNaturetr7").hide();
			$("#orgNatureLtr7").hide();
			$("#organizationCodeSF").attr("value",'');
			var organizationSFLicence=document.getElementById('organizationSFLicence');
			organizationSFLicence.innerHTML=organizationSFLicence.innerHTML.replace(/(value=\").+\"/i,"$1\"");
		}
		if(orgNature == 2){
			$("#orgNaturetr1").hide();
			$("#orgNatureLtr1").hide();
			$("#businessCode").attr("value",'');
			var businessLicence=document.getElementById('businessLicence');
			businessLicence.innerHTML=businessLicence.innerHTML.replace(/(value=\").+\"/i,"$1\"");
			$("#orgNaturetr2").show();
			$("#orgNatureLtr2").show();
			$("#organizationCode").attr("value",'');
			var organizationLicence=document.getElementById('organizationLicence');
